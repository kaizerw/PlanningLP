...
SOCRelaxationHeuristic::SOCRelaxationHeuristic(const options::Options &opts)
    : Heuristic(opts) {
    // Build propositions.
    int prop_id = 0;
    ...
    for (FactProxy fact : variables.get_facts()) {
        propositions[fact.get_variable().get_id()].push_back(
            Proposition(prop_id++));
    }
    ...
    // Use only valid operator
    vector<bool> valid_operators = opts.get<vector<bool>>("valid_operators");
    for (size_t op_id = 0; op_id < valid_operators.size(); ++op_id) {
        if (valid_operators[op_id]) {
            build_unary_operators(task_proxy.get_operators()[op_id], op_id);
        }
    }
    ////////////////////////////////////////////////////////////////////////////
...
SOCRelaxationHeuristic::~SOCRelaxationHeuristic() {}

bool SOCRelaxationHeuristic::dead_ends_are_reliable() const {
    return !task_properties::has_axioms(task_proxy);
}

Proposition *SOCRelaxationHeuristic::get_proposition(const FactProxy &fact) {
...
void SOCRelaxationHeuristic::build_unary_operators(const OperatorProxy &op,
                                                   int op_no) {
    ...
    for (EffectProxy effect : op.get_effects()) {
        ...
        unary_operators.push_back(
            UnaryOperator(precondition_props, effect_prop, op_no, base_cost));
        precondition_props.erase(precondition_props.end() - eff_conds.size(),
                                 precondition_props.end());
    }
}

void SOCRelaxationHeuristic::simplify() {
    ...
    */
    ...
    for (size_t i = 0; i < unary_operators.size(); ++i) {
        ...
        pair<Map::iterator, bool> inserted =
    ...
    for (Map::iterator it = unary_operator_index.begin();
        ...
        if (key.first.size() <= 5) {  // HACK! Don't spend too much time here...
            int powerset_size = (1 << key.first.size()) -
                                1;  // -1: only consider proper subsets
            for (int mask = 0; mask < powerset_size; ++mask) {
                Key dominating_key =
                    make_pair(vector<Proposition *>(), key.second);
                ...
                if (found != unary_operator_index.end()) {
                    int my_cost =
                        old_unary_operators[unary_operator_no].base_cost;
                    int dominator_op_no = found->second;
                    int dominator_cost =
                        old_unary_operators[dominator_op_no].base_cost;
    ...
    sort(unary_operators.begin(), unary_operators.end(),
         [&](const UnaryOperator &o1, const UnaryOperator &o2) {
             ...
             if (o1.effect != o2.effect) return o1.effect->id < o2.effect->id;
             if (o1.base_cost != o2.base_cost)
                 return o1.base_cost < o2.base_cost;
             return lexicographical_compare(
                 o1.precondition.begin(), o1.precondition.end(),
    ...
    // cout << " done! [" << unary_operators.size() << " unary operators]" <<
    // endl;
}
}  // namespace soc_relaxation_heuristic
