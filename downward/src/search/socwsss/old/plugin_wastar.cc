...
static shared_ptr<SearchEngine> _parse(OptionParser &parser) {
    ...
    shared_ptr<weager_search::WEagerSearch> engine;
    if (!parser.dry_run()) {
        ...
        engine = make_shared<weager_search::WEagerSearch>(opts);
    }

    return engine;
}

static Plugin<SearchEngine> _plugin("wastar", _parse);
}
