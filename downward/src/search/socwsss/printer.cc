...
Printer::Printer(OperatorsProxy ops, VariablesProxy vars,
                 shared_ptr<vector<shared_ptr<GLC>>> glcs,
                 chrono::time_point<chrono::system_clock> start)
    : ops(ops), vars(vars), glcs(glcs), start(start) {}
...
void Printer::update(int lp_h_oc, vector<long> rounded_solution, int c_size,
    ...
    this->plot_max_op_count.emplace_back(*max_element(
        rounded_solution.begin(), rounded_solution.begin() + ops.size()));
    this->plot_max_num_ops.emplace_back(accumulate(
        rounded_solution.begin(), rounded_solution.begin() + ops.size(), 0));
...
void Printer::show_data(int seq, double best_bound_found,
    ...
    transform(this->glcs->begin(), this->glcs->end(),
              back_inserter(ops_by_constraint), [&](shared_ptr<GLC> glc) {
                  return ((double)glc->ops_bounds.size()) / ops.size();
              });
    ...
    auto fn0 = [](int c) { return c == 0; };
    auto fn1 = [](int c) { return c == 1; };
    vector<int> cs;
    for (size_t i = 0; i < ops.size(); ++i) cs.emplace_back(ops[i].get_cost());
    vector<int> cs1;
    copy_if(cs.begin(), cs.end(), back_inserter(cs1), not_fn(fn0));

    cout << "Any zero cost ops: " << (int)any_of(cs.begin(), cs.end(), fn0)
         << endl;
    cout << "All unit cost ops: " << (int)all_of(cs.begin(), cs.end(), fn1)
         << endl;
    cout << "Min op cost: "
         << (cs1.size() > 0 ? *min_element(cs1.begin(), cs1.end()) : 0) << endl;
    cout << "Max op cost: "
         << (cs1.size() > 0 ? *max_element(cs1.begin(), cs1.end()) : 0) << endl;
    ...
    cout << "Last LP added constraints: "
         << (this->plot_lp_added_constraints.size() > 0
                 ...
                 : 0)
         << endl;
    cout << "Number of ops: " << ops.size() << endl;
    cout << "Number of variables: " << vars.size() << endl;
