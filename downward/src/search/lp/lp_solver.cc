...
void add_lp_solver_option_to_parser(OptionParser &parser) {
    ...
    parser.add_enum_option(
        "lpsolver", lp_solvers,
        "external solver that should be used to solve linear programs", "CPLEX",
        lp_solvers_doc);
}
...
bool LPConstraint::empty() const { return variables.empty(); }
...
void LPSolver::load_problem(LPObjectiveSense sense,
    ...
    try {
        CoinPackedMatrix matrix(false, variables.size(), constraints.size(),
                                elements.size(), elements.data(),
                                indices.data(), starts.data(), 0);
        lp_solver->loadProblem(matrix, col_lb.data(), col_ub.data(),
                               objective.data(), row_lb.data(), row_ub.data());
    ...
    if (!constraints.empty()) {
        ...
        for (const LPConstraint &constraint : constraints) {
            ...
            rows.push_back(new CoinShallowPackedVector(
                ...
                constraint.get_coefficients().data(), false));
        }

        try {
            lp_solver->addRows(num_rows, rows.data(), row_lb.data(),
                               row_ub.data());
        } catch (CoinError &error) {
            handle_coin_error(error);
        }
